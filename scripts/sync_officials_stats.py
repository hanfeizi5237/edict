...
def discover_guests(tasks):
    """发现诸侯（自定义 Agent）并计算统计"""
    guests = []
    cfg = _load_openclaw_cfg()
    court_ids = set(o['id'] for o in OFFICIALS) | set(ID_ALIAS.keys())
    ...
    for agent_dir in sorted(AGENTS_ROOT.iterdir()):
        ...
        if agent_id in court_ids:
            continue
        
        sj = agent_dir / 'sessions' / 'sessions.json'
        if not sj.exists():
            continue
        ...
        label = agent_id
        for a in cfg.get('agents',{}).get('list',[]):
            if a.get('id') == agent_id and a.get('label'):
                label = a['label']
                break
        
        # 计算统计
        ss = scan_agent(agent_id)
        model = get_model(agent_id)
        ts = get_task_stats(label, tasks)
